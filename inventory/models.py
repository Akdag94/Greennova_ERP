...
class Depo(models.Model):
    ...
    def mevcut_palet_sayisi(self):
        # Sadece depoda ve işlenmekte olanları doluluktan sayar
...
class Palet(models.Model):
    ...
    # YENİ ALAN: Banttan çıkan toplam palet sayısını tutar
    ...
    mulkiyet_tipi = models.CharField(max_length=20, choices=MULKIYET_SECENEKLERI, default='oz_mal', verbose_name="Mülkiyet")
    ...
    sevkiyat = models.ForeignKey('finance.Sevkiyat', on_delete=models.SET_NULL, null=True, blank=True, related_name='paletler', verbose_name="Yüklendiği Tır")
    
    # GÜNCELLEME: Esneklik için blank=True, null=True
    depo_konumu = models.ForeignKey(Depo, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Bulunduğu Oda")
    
    # --- MİKTAR MATEMATİĞİ ---
    brut_miktar_kg = models.DecimalField(max_digits=10, decimal_places=2, default=0, verbose_name="Kantar Giriş (Brüt KG)")
    
    # GÜNCELLEME: Esneklik için blank=True, null=True
    miktar_kg = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="Paketlenen / Stok (Net KG)")
    
    fire_miktar_kg = models.DecimalField(max_digits=10, decimal_places=2, editable=False, default=0, verbose_name="İşleme Farkı (Fire/Çıkma)")
    
    birim_fiyat = models.DecimalField(max_digits=10, decimal_places=2, default=0, verbose_name="Birim Fiyat (₺)")
    toplam_tutar = models.DecimalField(max_digits=12, decimal_places=2, editable=False, default=0, verbose_name="Toplam Tutar (Ödemeye Esas)")
    
    giris_tarihi = models.DateField(auto_now_add=True, verbose_name="Depoya Giriş Tarihi")
    
    # VARSAYILAN DURUM: 'isleniyor'
    durum = models.CharField(max_length=20, choices=DURUM_SECENEKLERI, default='isleniyor', verbose_name="Durum")
    ...
    def save(self, *args, **kwargs):
        # 1. ÖDEME MATEMATİĞİ (Kantar KG x Birim Fiyat)
        self.toplam_tutar = float(self.brut_miktar_kg) * float(self.birim_fiyat)
        
        # 2. İŞLEME FARKI HESABI (Kantar - Net)
...
def mustahsil_otomatik_cari_hareket(sender, instance, created, **kwargs):
    ...
    # Mükerrer ödemeyi ve 0 TL'lik çıkma malların cariye işlenmesini engeller
    if created and instance.toplam_tutar > 0:
