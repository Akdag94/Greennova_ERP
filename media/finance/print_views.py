...
def _render_pdf(template_name, context, dosya_adi):
    """Template'i render edip PDF döner."""
    context['logo_base64'] = _logo_base64()
    context['bugun'] = date.today()
    context['COMPANY_NAME'] = getattr(settings, 'COMPANY_NAME', 'GreenNova Tarım')
    context['COMPANY_ADDRESS'] = getattr(settings, 'COMPANY_ADDRESS', 'Bursa / Türkiye')
    context['COMPANY_PHONE'] = getattr(settings, 'COMPANY_PHONE', '')
    context['COMPANY_VERGI_NO'] = getattr(settings, 'COMPANY_VERGI_NO', '')
    context['COMPANY_VERGI_DAIRESI'] = getattr(settings, 'COMPANY_VERGI_DAIRESI', '')
    html = render_to_string(template_name, context)
    return _pdf_response(html, dosya_adi)
